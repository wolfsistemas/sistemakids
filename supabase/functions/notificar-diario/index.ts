...
async function configPush(sb: SupabaseClient): Promise<Record<string, boolean>> {
  const { data, error } = await sb.from("rk_push_config").select("chave, ativo");
  if (error) return {};
...
async function enviar(
  ...
  let query = sb
    .from("rk_push_subscriptions")
    .select("id, endpoint, p256dh, auth, pessoa_id, rk_pessoas!inner(id, is_user)");

  if (alvo.modo === "todos") {
    query = query.eq("rk_pessoas.is_user", true);
  } else {
  ...
  if (expirados.length) {
    await sb.from("rk_push_subscriptions").delete().in("id", expirados);
  }
...
async function notificarAniversariantes(sb: SupabaseClient, dryRun: boolean) {
  ...
  const { data: celulas } = await sb.from("rk_celulas").select("id, nome, lider_user_id").in("id", celulaIds);
...
serve(async (req) => {
  ...
  try {
    ...
    const aniversariantes = chaveAtiva(config, "aniversario")
      ...
      : { desativado: true };
    return json(200, { ok: true, dry_run: dryRun, aniversariantes });
  } catch (e) {
    return json(500, { error: String(e) });
  }
});
