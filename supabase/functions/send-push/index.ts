...
serve(async (req) => {
  ...
  const titulo = (body.titulo || body.title || "Células Kids").trim();
  ...
  let query = supabase
    ...
    .select("id, endpoint, p256dh, auth, pessoa_id, rk_pessoas!inner(id, categoria, is_user)");
  ...
  if (body.todos) {
    ...
    query = query.in("rk_pessoas.categoria", body.categorias);
  } else {
  ...
  if (expirados.length) {
    await supabase.from("rk_push_subscriptions").delete().in("id", expirados);
  }
